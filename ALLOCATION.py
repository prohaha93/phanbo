...
def run_optimization(file_input):
    ...
    print(f"Tổng số container yêu cầu: {total_demand_sum}")
    print("Chi tiết:")
    for k in sorted(total_demand):
        print(f"  WC={k[0]} ST={k[1]} POD={k[2]}: {total_demand[k]}")

    print("\n=== Tổng cung ===")
    total_supply_sum = sum(total_supply.values())
    print(f"Tổng số container có sẵn: {total_supply_sum}")
    ...
    # CẢI TIẾN 1: Hạn chế (block, bay, wc)
    block_bay_wc = {}
    for b in blocks:
        for bay in all_bays:
    ...
    excess_term = pulp.lpSum(excess.values())
    ...
    # Kiểm tra tổng số container đã phân bổ
    assigned_total = df_result['QUANTITIES'].sum()
    print(f"\n=== Tổng container đã phân bổ (theo mô hình): {assigned_total} ===")
    if assigned_total != total_demand_sum:
        print(f"CẢNH BÁO: Tổng phân bổ ({assigned_total}) khác tổng nhu cầu ({total_demand_sum})")
        print("Điều này có thể do mô hình không tìm được nghiệm tối ưu, hoặc có ràng buộc quá chặt.")

    # ============================================================
    # 5. Map container-level (nếu có)
    # ============================================================
    df_result_detail = []
    ...
    if container_data_available:
        ...
        print(f"\nContainer pool: {sum(len(lst) for lst in pool.values())} containers")
        ...
        for h in all_hours_sorted:
            ...
            for _, asg in hour_asgns.iterrows():
                ...
                if b not in pool:
                    # Không có container trong pool -> ghi dòng trống
        ...
        # Ghi các container không thể lấy (do hạn chế xếp chồng) vào kết quả với ID rỗng
        for d in deferred:
            df_result_detail.append({
                'MOVE HOUR': d['h_orig'],
                'STS': d['s'],
        ...
        print(f"Số dòng sau khi map container: {len(df_result_detail)}")
        print(f"Trong đó có {len(df_result_detail[df_result_detail['CONTAINER ID'] == ''])} dòng container trống (không lấy được container thực tế).")
    else:
        # Không có DATA sheet, giữ nguyên các dòng gộp
    ...
    # ============================================================
    # 6. Prepare MATRIX and DETAIL data (giữ nguyên)
    # ============================================================
    ...
    print(f"\nDone. Rows={total_rows}, Clashes={objective_value}")
