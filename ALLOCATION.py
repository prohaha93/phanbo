...
def run_optimization(file_input):
    ...
    # Build container-level stacking structures
    yb_wc_supply   = {}
    stack_ordering = {}
    blocking_pairs = []
    ...
    # ========== CẢI TIẾN 3: PHÂN BỔ ĐỒNG ĐỀU cho vessel bay lớn (MỚI) ==========
    # Tránh 1 block dồn quá nhiều container vào 1 BAY lớn (ví dụ 95 conts BAY 50 → B04)
    MAXLOAD_W = 0.01   # 0.02 = mạnh hơn, 0.005 = nhẹ hơn
    ...
    for vb in vessel_bay_keys:
        ...
        for b in blocks:
            ...
            if relevant_x:
                load_vars[(s, bay, b)] = pulp.LpVariable(f"load_{s}_{bay}_{b}", lowBound=0, cat='Integer')
    ...
    maxload_term      = pulp.lpSum(max_load_vars.values())
    ...
    # ============================================================
    # 4. Extract result + map individual containers
    # ============================================================
    result_rows = []
    ...
    if container_data_available:
        pool = {}
        for _, row in df_containers[['YARD','YB','YR','YT','REAL_WC','YARD_POS','REAL_CONT_ID','CONT_ST','CONT_POD']].iterrows():
            blk = row['YARD']
        ...
        def pick_n(block, wc, st_match, pod_match, qty, h, s_job, bay_job, h_rank_val, result_list):
            ...
            def matches(c):
                ...
                return True
            while remaining > 0:
                cands = [c for c in containers if not c['picked'] and matches(c) and accessible_at(c, containers, h_rank_val)]
                if not cands:
                    break
                yb_cnt = {}
                for c in cands:
    ...
    # ============================================================
    # 5~8. Prepare MATRIX, DETAIL, write Excel (giữ nguyên như cũ)
    # ============================================================
    ...
    for sts in sts_list:
        ...
        for bay in sts_bay_blocks[sts]:
            ...
            pivot = pivot.reindex(index=pd.MultiIndex.from_tuples([(h, w) for h, w in rows_idx.values]), columns=blks, fill_value=0)
            table_rows = []
    ...
    if container_data_available:
        ...
        all_result_cols = ['MOVE HOUR', 'STS', 'BAY', 'ASSIGNED BLOCK', 'WEIGHT CLASS', 'QUANTITIES']
    ...
    for st_idx, st_val in enumerate(st_values, 1):
            ...
            for i, (_, row) in enumerate(df_rd.iterrows()):
                ...
                if cur_key != prev_key:
                    if prev_key is not None:
                        merge_groups.append((prev_key, grp_start, excel_row - 1, cont_list_map.get(prev_key, '')))
                    prev_key = cur_key
                    grp_start = excel_row
            if prev_key is not None:
        ...
        for r_idx, (_, row) in enumerate(df_rd.iterrows(), 2):
            this_key = (row.get('MOVE HOUR'), row.get('STS'), row.get('BAY'), row.get('ASSIGNED BLOCK'), row.get('WEIGHT CLASS'))
        ...
        if cont_list_col_idx:
            for (mh, bay), r_start, r_end, list_text in merge_groups:
                ...
                cell.border = _thin_border()
                if r_end > r_start:
                    ws_result.merge_cells(start_row=r_start, start_column=cont_list_col_idx, end_row=r_end, end_column=cont_list_col_idx)
                for r in range(r_start, r_end + 1):
                    ws_result.row_dimensions[r].height = max(15, min(60, (len(list_text.split(',')) if list_text else 1) * 13))
    ...
    for sts in sts_list:
        next_row = write_detail_sts(ws_detail, sts, sts_col_start[sts], sts_table_widths[sts], detail_groups_by_sts[sts], sts_next_rows[sts])
